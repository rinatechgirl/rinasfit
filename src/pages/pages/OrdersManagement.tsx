...
import { Loader2, Package, Image as ImageIcon, User, RefreshCw } from "lucide-react";
...
const OrdersManagement = () => {
  ...
  const fetchOrders = async () => {
    ...
    // Fetch orders for this tenant that came from the customer portal
    ...
    // Fetch customer account info separately (no FK enforced)
    const customerIds = [
      ...new Set((rawOrders ?? []).map((o: any) => o.customer_user_id).filter(Boolean))
    ];
    ...
    if (customerIds.length > 0) {
      ...
      (accounts ?? []).forEach((a: any) => {
        accountMap.set(a.user_id, { full_name: a.full_name, email: a.email });
      });
    }
  ...
  const updateStatus = async (order: Order, newStatus: string) => {
    ...
    if (emailTemplate && order.customer_accounts?.email) {
      try {
        ...
        // Non-fatal — order still updated
        console.warn("Email notification failed:", emailErr);
      }
    }

    toast.success(`Order updated to "${newStatus}"${order.customer_accounts?.email ? " — customer notified." : "."}`);
    fetchOrders();
    setUpdatingId(null);
  };
  ...
  const setAgreedPrice = async (orderId: string, price: number) => {
    ...
    const { error } = await supabase.from("orders").update({ agreed_price: price } as any).eq("id", orderId);
        ...
        <Card className="border-border/60">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Publish designs to the magazine so customers can discover and order them.
            </p>
          </CardContent>
        </Card>
      ) : (
