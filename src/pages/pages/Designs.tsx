// NOTE: This file patches the fetchDesigns function in src/pages/Designs.tsx
// The only change from the original is:
//   1. fetchDesigns() explicitly filters by tenantId
//   2. handleSave() correctly passes tenant_id on insert
//
// Replace your full Designs.tsx with the content below.
...
import { Plus, Search, Pencil, Trash2, Image as ImageIcon, RotateCcw, Globe, EyeOff, Palette } from "lucide-react";
...
const Designs = () => {
  ...
  const fetchDesigns = async () => {
    // Always scope to the current tenant's designs only
    let query = supabase
      .from("designs")
      .select("*")
      .order("created_at", { ascending: false });

    // tenantId is the org's tenant. RLS also enforces this, but we add it
    // explicitly so the query is correct even if RLS is mis-configured.
    if (tenantId) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!tenantId) { toast.error("No organisation context. Please reload."); return; }
    setLoading(true);
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    ...
                    {editingId && designs.find(d => d.id === editingId)?.image_url && !frontFile && (
                      <p className="text-xs text-muted-foreground">Current image kept if no new file selected</p>
                    )}
                  </div>
                  <div className="space-y-2">
          ...
          <CardContent className="py-12 text-center text-muted-foreground">
            ...
            {canManage && (
              <Button variant="outline" size="sm" className="mt-3" onClick={() => setDialogOpen(true)}>
                <Plus className="w-3.5 h-3.5 mr-1" /> Add your first design
              </Button>
            )}
          </CardContent>
        </Card>
