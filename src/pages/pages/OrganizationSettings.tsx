...
import {
  ...
  CheckCircle2, UserPlus, Upload, X, Loader2, CreditCard,
} from "lucide-react";
...
const OrganizationSettings = () => {
  ...
  const handleLogoUpload = async () => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("tenant-logos").upload(fileName, logoFile, { upsert: true });
      if (uploadError) throw new Error(uploadError.message);
      const { data: urlData } = supabase.storage.from("tenant-logos").getPublicUrl(fileName);
      const { error: updateError } = await supabase.from("tenants").update({ logo_url: urlData.publicUrl } as any).eq("id", tenantId);
  ...
  const handleRemoveCurrentLogo = async () => {
    ...
    if (!confirm("Remove your organisation logo?")) return;
    setLogoUploading(true);
    try {
      const oldPath = (tenant as any).logo_url.split("/tenant-logos/")[1];
      if (oldPath) await supabase.storage.from("tenant-logos").remove([oldPath]);
  ...
  const fetchMembers = async () => {
    if (!tenantId) return;
    const { data: roles } = await supabase.from("user_roles").select("id, user_id, role").eq("tenant_id", tenantId);
    ...
    const merged = roles.map(r => {
      ...
      return { id: r.id, user_id: r.user_id, role: r.role, email: profile?.email || "", full_name: profile?.full_name || "" };
    });
    setMembers(merged);
  };
  ...
  const fetchInvitations = async () => {
    if (!tenantId) return;
    const { data } = await supabase.from("invitations").select("*").eq("tenant_id", tenantId).order("created_at", { ascending: false });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    else { toast.success("Organization details updated."); await refreshTenant(); }
    setSaving(false);
  };
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("invitations").insert({
      tenant_id: tenantId, email: inviteEmail.trim().toLowerCase(), role: inviteRole, invited_by: user.id,
    } as any);
    if (error) {
  ...
  const removeMember = async (member: TeamMember) => {
    ...
    await supabase.from("profiles").update({ tenant_id: null } as any).eq("user_id", member.user_id);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* ALL TabsTriggers must be inside TabsList */}
        ...
        <TabsContent value="general" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                This logo appears on your branded login page, dashboard, and sidebar.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                ...
                <div className="flex-1 space-y-3">
                    ...
                    {logoFile && (
                      <>
                        <Button type="button" size="sm" onClick={handleLogoUpload} disabled={logoUploading}>
                          {logoUploading ? <><Loader2 className="w-3.5 h-3.5 mr-2 animate-spin" />Uploading…</> : <><Save className="w-3.5 h-3.5 mr-2" />Save logo</>}
                        </Button>
                        <Button type="button" variant="ghost" size="sm" onClick={removeLogo} disabled={logoUploading}>
                          <X className="w-3.5 h-3.5 mr-1" /> Cancel
                        </Button>
                      </>
                    )}
                    {hasSavedLogo && !logoFile && (
                      <Button type="button" variant="ghost" size="sm" onClick={handleRemoveCurrentLogo} disabled={logoUploading} className="text-destructive hover:text-destructive">
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {logoFile && <span className="ml-2 text-foreground font-medium">"{logoFile.name}" selected — click Save logo to apply.</span>}
                  </p>
                </div>
            ...
            <CardContent>
              {members.length === 0 ? (
                ...
                <div className="space-y-3">
                  {members.map((m) => (
                    <div key={m.id} className="flex items-center justify-between p-3 rounded-lg border border-border bg-secondary/30">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <Badge className={m.role === "admin" ? "bg-accent/20 text-accent" : "bg-muted text-muted-foreground"}>
                          {m.role}
                        </Badge>
                      </div>
                    </div>
              ...
              <CardContent>
                <form onSubmit={handleInvite} className="flex flex-col sm:flex-row gap-3">
                  <div className="flex-1">
                    <Input type="email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} placeholder="colleague@email.com" required />
                  </div>
            ...
            <Card>
              ...
              <CardContent>
                {invitations.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {invitations.map((inv) => (
                      <div key={inv.id} className="flex items-center justify-between p-3 rounded-lg border border-border bg-secondary/30">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {inv.status === "pending" && (
                            <Button size="sm" variant="ghost" onClick={() => deleteInvitation(inv.id)}>
                              <Trash2 className="w-3.5 h-3.5 text-destructive" />
                            </Button>
                          )}
                        </div>
